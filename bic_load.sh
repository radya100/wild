...
export lb_file="logs/bic.mli"
export lb=$(<$lb_file)
export rows=49999
export drop_stage_sql=$(<"sql/bic/bic_drop_stage.sql")
export make_stage_sql=$(<"sql/bic/bic_make_stage.sql")
export select_file="sql/bic/bic_select.sql"
export select_sql=$(cat $select_file | tr '\n' ' ' \
| sed 's/_lb_/'$lb'/' \
| sed 's/_rows_/'$rows'/')
export insert_sql=$(<$"sql/bic/bic_insert.sql")
export fill_basic_file="sql/bic/bic_fill_basic.sql"
...
export pb_s=$(clickhouse-client --query="select toDate(min(dt)) as pb from stage_bic")
export pe_s=$(clickhouse-client --query="select toDate(max(dt)) as pe from stage_bic")
export mli=$(clickhouse-client --query="select max(log_id) from stage_bic")
export row=$(clickhouse-client --query="select count() from stage_bic")
...
if (( mli>0 )); then #Запись в основную таблицу и фиксирование log_id в файле
	...
	echo $mli>logs/bic.mli
fi
export end_ss=$(date +%s%3N)

#Логирование загрузки
export log_sql="insert into load_log(table_id, pb, event_date, rows, sec, sec_opt, msg) values(15, '$pb', '$(date -I)', $row, $(($begin_ss_opt-$begin_ss)), $(($end_ss-$begin_ss_opt)), '$msg')"
