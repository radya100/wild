...
export lb=$(($(clickhouse-client --query="select max(log_id) from ContainerChange where log_id > 0")))
export rows=49999
...
export row=$(clickhouse-client --query="select count(1) from ContainerChange where log_id>$lb")
