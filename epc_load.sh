...
export lb_file="logs/epc.mli"
export lb=$(<$lb_file)
export rows=50000
export drop_stage_sql=$(<"sql/epc/drop_stage.sql")
export make_stage_sql=$(<"sql/epc/make_stage.sql")
export select_file="sql/epc/select.sql"
export select_sql=$(cat $select_file | tr '\n' ' ' | sed 's/_lb_/'$lb'/' | sed 's/_rows_/'$rows'/')
export insert_sql=$(<$"sql/epc/insert.sql")
export fill_basic_file="sql/epc/fill_basic.sql"
...
#export pb_s=$(clickhouse-client --query="select toDate(min(start_dt)) as pb from stage_epc")
#export pe_s=$(clickhouse-client --query="select toDate(max(start_dt)) as pe from stage_epc")
export mli=$(clickhouse-client --query="select max(log_id) from stage_epc")
export row=$(clickhouse-client --query="select count() from stage_epc")
...
if (( mli>0 )); then #Запись в основную таблицу и фиксирование log_id в файле
	...
	echo $mli>logs/epc.mli
fi
export end_ss=$(date +%s%3N)

#Логирование загрузки
export log_sql="insert into load_log(table_id, pb, event_date, rows, sec, sec_opt, msg) values(21, '$pb', '$(date -I)', $row, $(($begin_ss_opt-$begin_ss)), $(($end_ss-$begin_ss_opt)), '$msg')"
