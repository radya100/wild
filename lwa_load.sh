...
export lb=$(($(clickhouse-client --query="select max(log_id) from laborer_work_assignment where log_id > 0")))
export rows=50000
export select_file="sql/lwa_select.sql"
export insert_file="sql/lwa_insert.sql"
...
clickhouse-client -u $user_ch --password $pas_ch --query="OPTIMIZE TABLE laborer_work_assignment"
export end_ss=$(date +%s%3N)

#Логирование загрузки
export row=$(clickhouse-client --query="select count(1) from laborer_work_assignment where log_id>$lb")
export log_sql="insert into load_log(table_id, pb, event_date, rows, sec, sec_opt, msg) values(14, '$pb', '$(date -I)', $row, $(($begin_ss_opt-$begin_ss)), $(($end_ss-$begin_ss_opt)), '$msg')"
