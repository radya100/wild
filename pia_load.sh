...
export lb=$(($(clickhouse-client --query="select max(log_id) from pia where log_id > 0")+1))
export rows=49999
export select_file="sql/pia_select.sql"
export insert_file="sql/pia_insert.sql"
...
clickhouse-client -u $user_ch --password $pas_ch --query="OPTIMIZE TABLE pia"
export end_ss=$(date +%s%3N)

#Логирование загрузки
export row=$(clickhouse-client --query="select count(1) from pia where log_id>$lb")
