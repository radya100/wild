...
#omega.conn
export lb=$(($(clickhouse-client --query="select max(log_id) from ShkLostChange")+1))
export rows=40000
export le=$(($lb+$rows))
export select_file="sql/slc_select.sql"
export insert_file="sql/slc_insert.sql"
...
clickhouse-client -u $user_ch --password $pas_ch --query="OPTIMIZE TABLE ShkLostChange"
export end_ss=$(date +%s%3N)

#Логирование загрузки
export row=$(clickhouse-client --query="select count(1) from ShkLostChange where log_id>$lb")
