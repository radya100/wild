...
export lb_file="logs/slc.mli"
export lb=$(<$lb_file)
export rows=50000
export le=$(($lb+$rows))
export drop_stage_sql=$(<"sql/slc/drop_stage.sql")
export make_stage_sql=$(<"sql/slc/make_stage.sql")
export select_file="sql/slc/select.sql"
export select_sql=$(cat $select_file | tr '\n' ' ' | sed 's/_lb_/'$lb'/' | sed 's/_rows_/'$rows'/' | sed 's/_le_/'$le'/')
export insert_sql=$(<$"sql/slc/insert.sql")
export fill_basic_file="sql/slc/fill_basic.sql"
...
#export pb_s=$(clickhouse-client --query="select toDate(min(dt)) as pb from stage_slc")
#export pe_s=$(clickhouse-client --query="select toDate(max(dt)) as pe from stage_slc")
export mli=$(clickhouse-client --query="select max(log_id) from stage_slc")
export row=$(clickhouse-client --query="select count() from stage_slc")
...
export log_sql="insert into load_log(table_id, pb, event_date, rows, sec, sec_opt, msg) values(1, '$pb', '$(date -I)', $row, $(($begin_ss_opt-$begin_ss)), $(($end_ss-$begin_ss_opt)), '$msg')"
