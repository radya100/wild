...
export lb=$(($(clickhouse-client --query="select max(sop_chg_id) from ShkOnPlaceChange where sop_chg_id > 4725022592")+1))
#printf "$lb"
export rows=49000
export le=$(($lb+$rows))
export select_file="sql/sop_select.sql"
export insert_file="sql/sop_insert.sql"
...
clickhouse-client -u $user_ch --password $pas_ch --query="OPTIMIZE TABLE ShkOnPlaceChange"
