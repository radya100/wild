...
export lb=$(($(clickhouse-client --query="select max(sop_chg_id) from ShkOnPlaceChange")+1))
#printf "$lb"
export rows=90000
export le=$(($lb+$rows))
export select_file="sql/sop_select_alice.sql"
...
clickhouse-client --query="OPTIMIZE TABLE ShkOnPlaceChange"
