...
export lb_file="logs/sop.mli"
export lb=$(<$lb_file)
export rows=50000
export le=$(($lb+$rows))
export drop_stage_sql=$(<"sql/sop/drop_stage.sql")
export make_stage_sql=$(<"sql/sop/make_stage.sql")
export select_file="sql/sop/select.sql"
export select_sql=$(cat $select_file | tr '\n' ' ' | sed 's/_lb_/'$lb'/' | sed 's/_le_/'$le'/' | sed 's/_rows_/'$rows'/')
export insert_sql=$(<$"sql/sop/insert.sql")
export fill_basic_file="sql/sop/fill_basic.sql"
...
export pb_s=$(clickhouse-client --query="select toDate(min(dt)) as pb from stage_sop")
export pe_s=$(clickhouse-client --query="select toDate(max(dt)) as pe from stage_sop")
export mli=$(clickhouse-client --query="select max(sop_chg_id) from stage_sop")
export row=$(clickhouse-client --query="select count() from stage_sop")
