...
export lb=$(($(clickhouse-client --query="select max(log_id) from StoragePlaceChange")))
export rows=49999
export select_file="sql/sp_select.sql"
export insert_file="sql/sp_insert.sql"
...
clickhouse-client -u $user_ch --password $pas_ch --query="OPTIMIZE TABLE StoragePlaceChange"
export end_ss=$(date +%s%3N)

#Логирование загрузки
export row=$(clickhouse-client --query="select count(1) from StoragePlaceChange where log_id>$lb")
export log_sql="insert into load_log(table_id, pb, event_date, rows, sec, sec_opt, msg) values(7, '$pb', '$(date -I)', $row, $(($begin_ss_opt-$begin_ss)), $(($end_ss-$begin_ss_opt)), '$msg')"
