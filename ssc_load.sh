...
export lb=$(($(clickhouse-client --query="select max(shk_state_chg_id) from ShkStateChange")+1))
#printf "$lb"
export rows=90000
export le=$(($lb+$rows))
export select_file="sql/ssc_select_alice.sql"
...
clickhouse-client --query="OPTIMIZE TABLE ShkStateChange"
