...
export lb_file="logs/tb.mli"
export lb=$(<$lb_file)
export rows=49999
export le=$(($lb+$rows))

export drop_stage_sql=$(<"sql/tb/tb_drop_stage.sql")
export make_stage_sql=$(<"sql/tb/tb_make_stage.sql")
export select_file="sql/tb/tb_select.sql"
export select_sql=$(cat $select_file | tr '\n' ' ' | sed 's/_lb_/'$lb'/' | sed 's/_le_/'$le'/' | sed 's/_rows_/'$rows'/')
export insert_sql=$(<$"sql/tb/tb_insert.sql")
export fill_basic_file="sql/tb/tb_fill_basic.sql"
...
export pb_s=$(clickhouse-client --query="select toDate(min(dt)) as pb from stage_TransferBoxChange")
export pe_s=$(clickhouse-client --query="select toDate(max(dt)) as pe from stage_TransferBoxChange")
export mli=$(clickhouse-client --query="select max(log_id) from stage_TransferBoxChange")
export row=$(clickhouse-client --query="select count() from stage_TransferBoxChange")
